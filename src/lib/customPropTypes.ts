 ...
 */
export const domNode = (props: any, propName: any) => {
  ...
  return new Error(`Invalid prop "${String(propName)}" supplied, expected a DOM node.`);
};
 ...
 */
export const suggest = (suggestions: any) => {
  ...
  const findBestSuggestions = _.memoize((str) => {
    ...
    return _.flow(
      _.map((suggestion: any) => {
        const suggestionWords = suggestion.split(' ');

        const propValueScore = _.flow(
          _.map((x: string) => _.map((y: string) => leven(x, y), suggestionWords)),
          _.map(_.min),
          _.sum,
        )(propValueWords);

        const suggestionScore = _.flow(
          _.map((x: string) => _.map((y: string) => leven(x, y), propValueWords)),
          _.map(_.min),
          _.sum,
        )(suggestionWords);
  ...
  return (props: any, propName: any, componentName: any) => {
    ...
    return new Error(
      [
        `Invalid prop \`${String(propName)}\` of value \`${propValue}\` supplied to \`${componentName}\`.`,
        `\n\nInstead of \`${propValue}\`, did you mean:`,
        bestMatches.map((x) => `\n  - ${(x as any).suggestion}`).join(''),
        '\n',
      ].join(''),
    );
  };
 ...
 */
export const disallow = (disallowedProps: string[]) => (props: any, propName: any, componentName: any) => {
  if (!Array.isArray(disallowedProps)) {
    throw new Error(
      [
        'Invalid argument supplied to disallow, expected an instance of array.',
        ` See \`${String(propName)}\` prop in \`${componentName}\`.`,
      ].join(''),
    );
  }
  ...
  if (disallowed.length > 0) {
    return new Error(
      [
        `Prop \`${String(propName)}\` in \`${componentName}\` conflicts with props: \`${disallowed.join(
          '`, `',
        )}\`.`,
 ...
 */
export const every = (validators: any[]) => (props: any, propName: any, componentName: any, ...rest: any[]) => {
 ...
 * @param {function[]} validators An array of propType functions.
 */
export const some = (validators: any[]) => (props: any, propName: any, componentName: any, ...rest: any[]) => {
  if (!Array.isArray(validators)) {
    throw new Error(
      [
  ...
  const errors = _.compact<any>(
    _.map<any>(validators, (validator: any) => {
  ...
  if (errors.length === validators.length) {
    ...
    error.message += `\n${_.map<any>(errors, (err: any, i: number) => `[${i + 1}]: ${err.message}`).join('\n')}`;
    return error;
  }
};
 ...
 */
export const givenProps = (propsShape: any, validator: any) => (props: any, propName: any, componentName: any, ...rest: any[]) => {
 ...
 */
export const demand = (requiredProps: string[]) => (props: any, propName: any, componentName: any) => {
  ...
  if (missingRequired.length > 0) {
    return new Error(
      `\`${String(propName)}\` prop in \`${componentName}\` requires props: \`${missingRequired.join(
        '`, `',
      )}\`.`,
    );
  }
 ...
 */
export const multipleProp = (possible: string[]) => (props: any, propName: any, componentName: any) => {
  ...
  const values = propValue
    ...
    .split(' ')
    .map((val: string) => _.trim(val).replace('-', ' '));
  const invalid = _.difference<any>(values, possible) as any;

  // fail only if there are invalid values
  if (invalid.length > 0) {
    return new Error(
      `\`${String(propName)}\` prop in \`${componentName}\` has invalid values: \`${invalid.join(
        '`, `',
      )}\`.`,
    );
  }
 ...
 */
export const contentShorthand = (arg0: any, arg1: any, arg2: any, ...args: any[]) =>
  every([disallow(['children']), PropTypes.node])(arg0, arg1, arg2, ...args);

/**
 * Item shorthand is a description of a component that can be a literal,
 * a props object, or an element.
 */
export const itemShorthand = (arg0: any, arg1: any, arg2: any, ...args: any[]) =>
  every([
    disallow(['children']),
    PropTypes.oneOfType([
      PropTypes.func,
 ...
 */
export const collectionShorthand = (arg0: any, arg1: any, arg2: any, ...args: any[]) =>
  every([disallow(['children']), PropTypes.arrayOf(itemShorthand)])(arg0, arg1, arg2, ...args);
 ...
 * @param {function} [validator] A propType function.
 */
export const deprecate = (help: any, validator: any) => (props: any, propName: any, componentName: any, ...args: any[]) => {
  ...
  const error = new Error(`The \`${String(propName)}\` prop in \`${componentName}\` is deprecated.`);
